...
func (this *Ip2Region) BtreeSearch(ipStr string) (ipInfo IpInfo, err error) {
	...
	blockLen := eptr - sptr
	_,_ = this.dbFileHandler.Seek(sptr, 0)
	index := make([]byte, blockLen+INDEX_BLOCK_LENGTH)
	_,_ = this.dbFileHandler.Read(index)
	var dataptr int64
	...
	_,_ = this.dbFileHandler.Seek(dataPtr, 0)
	data := make([]byte, dataLen)
	_,_ = this.dbFileHandler.Read(data)
